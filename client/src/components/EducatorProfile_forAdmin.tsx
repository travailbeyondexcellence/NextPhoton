...
const EducatorProfile_forAdmin = () => {
    return (
        <div className="min-h-screen bg-slate-300 dark:bg-[#0f172a] text-white p-6 space-y-6 rounded-xl  min-w-800 mx-auto max-w-[1200px]">
          {/* Header */}
          <div className="text-2xl font-semibold flex items-center gap-2">
            <span className="bg-slate-800 p-2 rounded-full">👤</span>
            Profile
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-slate-800 rounded-xl p-6 flex items-center gap-6">
              <Image
                ...
                width={120}
                height={120}
                className="rounded-xl object-cover border-4 border-blue-500"
              />
              <div>
                <h2 className="text-xl font-semibold">{educator.name}</h2>
                <p className="text-sm text-blue-400">{educator.username}</p>
                <p className="mt-2 text-sm text-gray-300">{educator.intro}</p>
              </div>
            </div>
          ...
          <div className="bg-slate-800 p-6 rounded-xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-center">
              <div>
                ...
                <p className="text-sm text-gray-400">Years with NextPhoton</p>
              </div>
              <div>
            ...
            <div className="space-y-4">
              {educatorReviews.map((review, index) => (
                <div key={index} className="bg-slate-700 p-4 rounded-md">
                  ...
                  <div className="text-xs text-gray-400 mb-2">{review.date} ({review.exam})</div>
            ...
            <ul className="space-y-3 text-sm text-gray-300">
              {adminNotes.map((note, idx) => (
                <li key={idx} className="border-l-4 border-blue-500 pl-4">
                  ...
                  <p className="text-xs text-gray-500 mb-1">{note.timestamp}</p>
                  <p>{note.note}</p>
                </li>
              ))}
            </ul>
