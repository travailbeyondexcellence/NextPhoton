...
const getPriceTagColor = (tier: string) => {
  switch (tier.toLowerCase()) {
    case "beginner":
      return "bg-[#f7d749] text-black";
    case "intermediate":
      return "bg-[#3389ea] text-white";
    case "premium":
...
const EducatorCard_forAdmin = () => {
  return (
    <div className="relative flex flex-col md:flex-row bg-background text-foreground rounded-xl overflow-hidden shadow-lg border border-muted dark:bg-gray-900 dark:text-gray-100 dark:border-gray-800 w-full max-w-4xl cursor-pointer hover:scale-[1.015] transition-transform duration-300">
      ...
      <div className={`absolute top-2 right-2 text-xs font-semibold px-3 py-1 rounded ${getPriceTagColor(educator.priceTier)}`}>
      ...
      <div className="flex-1 p-6 space-y-2">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm">
          ...
          <span>
            <span className="font-medium">Exam Tags:</span>{" "}
            {educator.exams.join(", ")}
          </span>
        </div>
      ...
      {/* Floating bottom-right subjects label */}
      <div className="absolute bottom-2 right-3 text-sm font-semibold">
